...
namespace ContractMvcWeb.Utils
{
    public class ExcelUtils
    {
        ...
        public static MemoryStream RenderToExcel(List<Models.Beans.ContractCG> list)
        {
            ...
            CreateCell(row, 6, "采购类型", CellType.String, style);
            CreateCell(row, 7, "采购内容", CellType.String, style);                       
            CreateCell(row, 8, "数量", CellType.String, style);
            CreateCell(row, 9, "单价", CellType.String, style);
            CreateCell(row, 10, "小计", CellType.String, style);
            CreateCell(row, 11, "总价", CellType.String, style);
            CreateCell(row, 12, "验收情况", CellType.String, style);
            CreateCell(row, 13, "登记位置", CellType.String, style);
            CreateCell(row, 14, "合同编号", CellType.String, style);            
            ...
            CreateCell(row, 20, "分管部门", CellType.String, style);
            CreateCell(row, 21, "备注", CellType.String, style);
            CreateCell(row, 22, "支付进度", CellType.String, style);
            CreateCell(row, 23, "预算金额", CellType.String, style);
            CreateCell(row, 24, "经费来源", CellType.String, style);
            CreateCell(row, 25, "结算金额", CellType.String, style);
            #endregion
            ...
            if (list != null && list.Count > 0)
            {
                ...
                foreach (Models.Beans.ContractCG model in list)
                {
                    ...
                    CreateCell(row, 6, model.type);
                    CreateCell(row, 7, model.content);                                                
                    CreateCell(row, 8, model.count.ToString());
                    CreateCell(row, 9, model.price.ToString());
                    CreateCell(row, 10, model.subtotal.ToString());
                    CreateCell(row, 11, model.total.ToString());
                    CreateCell(row, 12, model.validate);
                    CreateCell(row, 13, model.place);
                    CreateCell(row, 14, model.contractnum);                   
                    ...
                    CreateCell(row, 20, model.chargedepartment);
                    CreateCell(row, 21, model.remark);
                    CreateCell(row, 22, model.payprogress);
                    CreateCell(row, 23, model.budgetamount);
                    CreateCell(row, 24, model.fundsource);                    
                    CreateCell(row, 25, model.settleamount);
                    #endregion
                }
            }
            workbook.Write(ms);
        ...
        public static MemoryStream RenderToExcel(List<Models.Beans.ContractLX> list)
        {
            ...
            CreateCell(row, 6, "采购类型", CellType.String, style);
            CreateCell(row, 7, "采购内容", CellType.String, style);
            CreateCell(row, 8, "单价", CellType.String, style);
            CreateCell(row, 9, "数量", CellType.String, style);
            CreateCell(row, 10, "总价", CellType.String, style);
            CreateCell(row, 11, "采购合同", CellType.String, style);
            CreateCell(row, 12, "采购纪要", CellType.String, style);
            CreateCell(row, 13, "验收情况", CellType.String, style);
            ...
            CreateCell(row, 17, "分管部门", CellType.String, style);
            CreateCell(row, 18, "备注", CellType.String, style);
            CreateCell(row, 19, "支付进度", CellType.String, style);
            CreateCell(row, 20, "结算金额", CellType.String, style);
            CreateCell(row, 21, "赠送内容", CellType.String, style);
            CreateCell(row, 22, "赠送价值", CellType.String, style);            
            CreateCell(row, 23, "记录位置", CellType.String, style);
                        
            CreateCell(row, 24, "预算金额", CellType.String, style);
            CreateCell(row, 25, "经费来源", CellType.String, style);
            #endregion
            ...
            if (list != null && list.Count > 0)
            {
                ...
                foreach (Models.Beans.ContractLX model in list)
                {
                    ...
                    CreateCell(row, 6, model.type);
                    CreateCell(row, 7, model.content);
                    CreateCell(row, 8, model.price.ToString());
                    CreateCell(row, 9, model.count.ToString());
                    CreateCell(row, 10, model.total.ToString());
                    CreateCell(row, 11, model.contractnum);
                    CreateCell(row, 12, model.summaryofbuy);
                    CreateCell(row, 13, model.validate);                    
                    CreateCell(row, 14, model.super);
                    CreateCell(row, 15, model.superlinker);
                    CreateCell(row, 16, model.supertel);
                    CreateCell(row, 17, model.chargedepartment);
                    CreateCell(row, 18, model.remark);
                    CreateCell(row, 19, model.payprogress);
                    CreateCell(row, 20, model.settleamount);
                    CreateCell(row, 21, model.freecontent);
                    CreateCell(row, 22, model.freevalue);                    
                    CreateCell(row, 23, model.place);
                    CreateCell(row, 24, model.budgetamount);   
                    CreateCell(row, 25, model.fundsource);
